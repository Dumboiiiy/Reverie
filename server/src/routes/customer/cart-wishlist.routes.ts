...
import { getDbUserFromReq, requireAuth } from "../../middlewares/auth";
...
customerCartWishlistRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json(success(await getCartResponse(String(dbUser._id))));
  }),
);
...
customerCartWishlistRouter.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!cart) {
      res.json(success({ items: [], totalQuantity: 0 }));
      return;
    }
...
customerCartWishlistRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    res.json(success(await getWishlistResponse(String(dbUser._id))));
  }),
);
