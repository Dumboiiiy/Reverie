...
export function CustomerNavbar() {
  ...
  const isAdmin = Boolean(isSignedIn && user?.role === "admin");
    ...
    <header className={headerClass}>
      <div className={shell}>
        <Link to={"/"} className={brandWrap}>
          ...
          />
          <span className={brandTitle}>Reverie</span>
        </Link>
          ...
          {isAdmin ? (
            <Link to="/admin">
              <Button
                variant="outline"
                size="sm"
                className="h-10 rounded-xl border-amber-500/30 bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 hover:text-amber-300 font-medium flex items-center gap-2"
              >
